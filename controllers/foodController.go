...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		recordPerPage, err := strconv.Atoi(c.Query("recordPerPage")) // records per page
		if err != nil || recordPerPage < 1 {
			recordPerPage = 10
		}
		page, err := strconv.Atoi(c.Query("page")) //page number
		if err != nil || page < 1 {
			page = 1
		}
		...
		if err != nil{
			msg := fmt.Sprintf("error occured while listing food items")
...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//checking if item present in menu
...
func UpdateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err !=nil{
			msg := fmt.Sprintf("update query failed")
			c.JSON(html.StatusInternalServerError, gin.H{"error":msg})
			return
		}
		c.JSON(html.StatusOK, result)
	}
}
...
func toFixed(num float64, precision int) float64 {
	out:=mat.Pow(10, float64(precision))
